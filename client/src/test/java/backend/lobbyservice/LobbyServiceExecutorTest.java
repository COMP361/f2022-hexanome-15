...
class LobbyServiceExecutorTest {
	...
	void runLSDebug() {
		...
		String out = ParseOutput.parseText(runScript.getProcess());
		System.out.println(out);
		
		try {
			assertEquals(0, runScript.getProcess().waitFor());
	...
	void runLSGetAuth() throws InterruptedException {
		RunScript runScript = new RunScript(
				...
				"maex", "abc123_ABC123");
		ls.execute(runScript);
		JSONObject out = ParseOutput.parseJson(runScript.getProcess());
		System.out.println(out);
		assertEquals(0, runScript.getProcess().waitFor());
		runScript.destroy();
	}
