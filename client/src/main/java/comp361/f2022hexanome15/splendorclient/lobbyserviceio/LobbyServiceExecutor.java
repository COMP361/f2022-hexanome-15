...
public class LobbyServiceExecutor {
	...
	public final JSONObject auth_role(String accessToken) {
		String command = String.format("curl -X GET %s/oauth/role?access_token=%s", lobbyServiceLocation, accessToken);
	 ...
	 */
	public final long create_session(String accessToken, String createUserName, String gameName, String saveGame) {
	...
	public final void launch_session(String accessToken, long sessionid) {
		...
		String command = String.format("curl -X POST %s/api/sessions/%d?access_token=%s", lobbyServiceLocation,
	...
	public final void register_gameservice(String accessToken, String gameLocation, int maxSessionPlayers,
		...
		String command = String.format(
				...
				String.valueOf(maxSessionPlayers), String.valueOf(webSupport), lobbyServiceLocation, accessToken);
		run(command, NullParser.NULLPARSER);
	}
	...
	public final void remove_session(String accessToken, long sessionid) {
		checkNotNullNotEmpty(accessToken);
		String command = String.format("curl -X DELETE %s/api/sessions/%d?access_token=%s", lobbyServiceLocation,
				String.valueOf(sessionid), accessToken);
		run(command, NullParser.NULLPARSER);
	}
	...
	public final void unregister_gameservice(String gameName, String accessToken) {
		...
		String command = String.format("curl -X DELETE %s/api/gameservices/%s?access_token=%s", lobbyServiceLocation,
	...
	public final JSONObject get_user(String username, String accessToken) {
		...
		String command = String.format("curl -X GET %s/api/users/%s?access_token=%s", lobbyServiceLocation, username,
				accessToken);
	...
	private Object run(String command, OutputParser parser) {
		try {
			...
			if (exitCode != 0) {
				...
				throw new RuntimeException("[WARNING] Process: " + command + " resulted in exit code: " + exitCode);
			}
