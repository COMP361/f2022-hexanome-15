...
public class GameController
{
    ...
    private final Object register_gameservice(String accessToken, String gameLocation,
                                              ...
                                              int minSessionPlayers, String gameName, String displayName,
                                              boolean webSupport
    )
    {
    ...
    {
        Optional<GameBoardManager> manager = BroadcastManager.getActiveGame(gameid);
        if (manager.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        } else {
            String json = new Gson().toJson(manager.get().getBoard());
